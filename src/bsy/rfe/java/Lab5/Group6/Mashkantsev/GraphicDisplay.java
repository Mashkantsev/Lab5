...
public class GraphicDisplay extends JPanel {
    ...
    public class MouseHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent ev) {
            if (ev.getButton() == 1) {
                GraphicDisplay.this.selectedMarker = GraphicDisplay.this.findSelectedPoint(ev.getX(), ev.getY());
                GraphicDisplay.this.originalPoint = GraphicDisplay.this.translatePointToXY(ev.getX(), ev.getY());
                if (GraphicDisplay.this.selectedMarker >= 0) {
                    GraphicDisplay.this.changeMode = true;
                    GraphicDisplay.this.setCursor(Cursor.getPredefinedCursor(8));
                } else {
        ...
        public void mouseReleased(MouseEvent ev) {
            if (ev.getButton() == 1) {
                GraphicDisplay.this.setCursor(Cursor.getPredefinedCursor(0));
                if (GraphicDisplay.this.changeMode) {
                    GraphicDisplay.this.changeMode = false;
                } else {
                    GraphicDisplay.this.scaleMode = false;
                    double[] finalPoint = GraphicDisplay.this.translatePointToXY(ev.getX(), ev.getY());
                    GraphicDisplay.this.undoHistory.add(GraphicDisplay.this.viewport);
                    GraphicDisplay.this.viewport = new double[2][2];
                    GraphicDisplay.this.zoomToRegion(GraphicDisplay.this.originalPoint[0], GraphicDisplay.this.originalPoint[1], finalPoint[0], finalPoint[1]);
                    GraphicDisplay.this.repaint();
                }
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            if (GraphicDisplay.this.changeMode) {
                double[] currentPoint = GraphicDisplay.this.translatePointToXY(ev.getX(), ev.getY());
                double newY = ((Double[])GraphicDisplay.this.graphicsData.get(GraphicDisplay.this.selectedMarker))[1] + (currentPoint[1] - ((Double[])GraphicDisplay.this.graphicsData.get(GraphicDisplay.this.selectedMarker))[1]);
                if (newY > GraphicDisplay.this.viewport[0][1]) {
                    newY = GraphicDisplay.this.viewport[0][1];
                }

                if (newY < GraphicDisplay.this.viewport[1][1]) {
                ...
                double height = (double)ev.getY() - GraphicDisplay.this.selectionRect.getY();
                if (height < 5.0D) {
                    height = 5.0D;
                }

                GraphicDisplay.this.selectionRect.setFrame(GraphicDisplay.this.selectionRect.getX(), GraphicDisplay.this.selectionRect.getY(), width, height);
                GraphicDisplay.this.repaint();
            }
